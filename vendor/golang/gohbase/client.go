...
import (
	...
	"golang/gohbase/hrpc"
	"golang/gohbase/pb"
	"golang/gohbase/region"
	"golang/gohbase/regioninfo"
	"golang/gohbase/zk"
	log "golang/log4go"
...
func (c *client) SetServer(resourceType int, ms *zk.ServerInfo) {
	...
	case zk.ResourceTypeMeta:
		...
		go mc.Close()
	default:
		log.Warn("unrecognized resourceType: %d", resourceType)
	}
}
...
func newClient(clientType int, c *conf.Conf, options ...Option) *client {
	log.Debug("Creating new client. Host: %v", c.Zkquorum)
	if c.FlushInterval <= 0 && c.RpcQueueSize < 0 {
		log.Error("flushInterval (%d) <= 0 and queueSize (%d) < 0", int64(c.FlushInterval), c.RpcQueueSize)
		return nil
	}
	cl := &client{
	...
	if err != nil {
		log.Error("zk.NewZKClient(%v, %s, %s) failed, err is (%v)", c.Zkquorum, c.Master, c.Meta, err)
		return nil
	}
...
func (c *client) Go(cs *hrpc.Calls) (res []CallResult) {
	...
	for i := 0; i < callsNum; i++ {
		...
		case <-cs.Ctx.Done():
			log.Warn("timeout when do Go for (%d) calls", callsNum)
			return
		}
	}
	return
...
func (c *client) sendRPCToRegion(rpc hrpc.Call, reg *regioninfo.Info) (proto.Message, error) {
	...
	if err != nil {
		...
		if _, ok := err.(region.UnrecoverableError); ok {
			...
			// considered dead.
			log.Error("met UnrecoverableError (%v) when access region (%v)", err, reg)
			...
			log.Error("met error (%v) when clientFor reg or queueRPC, try to reestablish region (%v)", err, reg)
	...
	if _, ok := res.Error.(region.RetryableError); ok {
		...
		// it.
		log.Error("met RetryableError (%v) when access region (%v)", res.Error, reg)
		first := reg.MarkUnavailable()
		if first {
			go c.reestablishRegion(reg)
		}
		...
		// If it was an unrecoverable error, the region client is
		// considered dead.
		log.Error("met UnrecoverableError (%v) when access region (%v)", res.Error, reg)
		if reg == c.metaRegionInfo || reg == c.adminRegionInfo {
			// If this is the admin client or the meta table, mark the
			// region as unavailable and start up a goroutine to
			// reconnect if it wasn't already marked as such.
...
func (c *client) findRegionAndSendRPC(rpc hrpc.Call) (proto.Message, error) {
	...
	for {
		...
		log.Info("region cache miss for (%q) (%q)", rpc.Table(), rpc.Key())
...
func (c *client) locateRegion(ctx context.Context,
	...
	log.Info("locate region for table (%q), key (%q)", table, key) // for test
	...
	if err != nil {
		log.Error("get meta for metaKey (%s) met error(%v)", metaKey, err)
		return nil, "", 0, err
	}
	...
	log.Info("locate region for table (%q), key (%q), found: (%v), (%s), (%d)", table, key, reg, host, port) // for test
...
func (c *client) establishRegion(originalReg *regioninfo.Info, host string, port uint16) {
	log.Info("establishRegion(%v, %s, %d)", originalReg, host, port)
	...
	for {
		...
		if port != 0 && err == nil {
			...
			case <-ctx.Done():
				log.Warn("region lookup timeout for clientType(%d), host(%d), port(%d)", clientType, host, port)
				err = ErrDeadline
			}
		}
		if err != nil {
