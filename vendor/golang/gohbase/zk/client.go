	...
	for i := 0; i < resourceCount; i++ {
		...
		go func(resourceType int, wg *sync.WaitGroup) {
			for {
				...
				if getErr != nil {
					log.Error("c.conn.GetW(%s) failed, err is (%v)", path, getErr)
					if wg != nil {
						err = getErr
						wg.Done()
						return
					...
					if newServerInfo == nil {
						log.Error("serverInfoFromContent(%v, %d) return nil serverInfo", buf, resourceType)
...
func (c *ZKClient) LocateResource(resourceType int) (res *ServerInfo) {
	...
	log.Debug("LocateResource(%d), return %v", resourceType, res)
	return
}

func serverInfoFromContent(buf []byte, resourceType int) (res *serverInfo) {
	if len(buf) == 0 {
		log.Error("%d was empty!", resourceType)
		return
	} else if buf[0] != 0xFF {
		log.Error("The first byte of %d was 0x%x, not 0xFF", resourceType, buf[0])
		return
	}
